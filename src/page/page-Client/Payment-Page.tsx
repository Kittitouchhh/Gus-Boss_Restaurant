...
const PaymentPage: React.FC = ({ }) => {
    ...
    useEffect(() => {
        ...
        const users = JSON.parse(localStorage.getItem("users") || "[]");
        const foundUser = users.find((u: any) => u.id?.toString() === currentId);
    ...
    useEffect(() => {
        ...
        if (user.membership.isMember === false) {
            setDiscountpercentage(0);
            return;
        }

        const infoMember = CalculateMembership(user.membership.points);
        setDiscountpercentage(infoMember.discount);
